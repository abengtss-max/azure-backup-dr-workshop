...
3. Under **Step 1: Select restore point**, select the **Select** link next to **Restore point**. In the **Select restore point** window, adjust **Start Date / End Date** if needed, select the pre-staged recovery point row (e.g. the **Application Consistent** point), and choose **OK**. Note its UTC timestamp on your evidence sheet.
